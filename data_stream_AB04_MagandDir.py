...
with open('data_stream_AB04', 'rb') as f:
...
#         A_dir_mag = cpoint-(cpmean-2*cpstd)
                ...
                if tracking_active:
                    ...
                    if heel_strike_count>14:
                        ...
                        for compare_index in range (100):
                            # if early_flag ==1:
                            #     break
                            PCX,Amdcx,cxmean = A_valuevel(mean_com[compare_index,0],std_com[compare_index,0],up_cyc_c[compare_index,0])
                            PCY,Amdcy,cymean = A_valuevel(mean_com[compare_index,1],std_com[compare_index,1],up_cyc_c[compare_index,1])

                            PRX,Amdrx,rxmean = A_valuevel(mean_rhl[compare_index,0],std_rhl[compare_index,0],up_cyc_r[compare_index,0])
                            PRY,Amdry,rymean = A_valuevel(mean_rhl[compare_index,1],std_rhl[compare_index,1],up_cyc_r[compare_index,1])
                            PRZ,_,_ = A_valuevel(mean_rhl[compare_index,2],std_rhl[compare_index,2],up_cyc_r[compare_index,2])

                            PLX,Amdlx,lxmean = A_valuevel(mean_lhl[compare_index,0],std_lhl[compare_index,0],up_cyc_l[compare_index,0])
                            PLY,Amdly,lymean = A_valuevel(mean_lhl[compare_index,1],std_lhl[compare_index,1],up_cyc_l[compare_index,1])
                            PLZ,_,_ = A_valuevel(mean_lhl[compare_index,2],std_lhl[compare_index,2],up_cyc_l[compare_index,2])

                            PCXV,_,_ = A_valuevel(mean_comv[compare_index,0],std_comv[compare_index,0],up_cyc_cv[compare_index,0])
                            PCYV,_,_ = A_valuevel(mean_comv[compare_index,1],std_comv[compare_index,1],up_cyc_cv[compare_index,1])

                            PRXV,_,_ = A_valuevel(mean_rhlv[compare_index,0],std_rhlv[compare_index,0],up_cyc_rv[compare_index,0])
                            PRYV,_,_ = A_valuevel(mean_rhlv[compare_index,1],std_rhlv[compare_index,1],up_cyc_rv[compare_index,1])
                            PRZV,_,_ = A_valuevel(mean_rhlv[compare_index,2],std_rhlv[compare_index,2],up_cyc_rv[compare_index,2])

                            PLXV,_,_ = A_valuevel(mean_lhlv[compare_index,0],std_lhlv[compare_index,0],up_cyc_lv[compare_index,0])
                            PLYV,_,_ = A_valuevel(mean_lhlv[compare_index,1],std_lhlv[compare_index,1],up_cyc_lv[compare_index,1])
                            PLZV,_,_ = A_valuevel(mean_lhlv[compare_index,2],std_lhlv[compare_index,2],up_cyc_lv[compare_index,2])
                            ...
                            if Psum > 0.125:
                                ...
                                if trial >=10:
                                    ...
                                    print ("Trip")
                                print("perted")
                                print(f"COMx: {Amdcx*100}")
                                print(f"Mean: {cxmean*100}")
                                print(f"COMy: {Amdcy*100}")
                                print(f"Mean: {cymean*100}")
                                print(f"RHLx: {Amdrx*100}")
                                print(f"Mean: {rxmean*100}")
                                print(f"RHLy: {Amdry*100}")
                                print(f"Mean: {rymean*100}")
                                print(f"LHLx: {Amdlx*100}")
                                print(f"Mean: {lxmean*100}")
                                print(f"LHLy: {Amdly*100}")
                                print(f"Mean: {lymean*100}")
                                print(f"Psum: {Psum}")
